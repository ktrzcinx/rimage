...
static int assert_everything_parsed(const toml_table_t *table, struct parse_ctx *ctx)
{
	...
	/* from number of parsed fields substract fields count in given table */
		      ...
		      bool verbose)
{
	...
	out->image_size = parse_uint32_hex_key(adsp, &ctx, "image_size", 0x100000, &ret);
	if (ret < 0)
		return ret;
			 ...
			 struct sof_man_fw_desc *out, bool verbose)
{
	...
	out->header.preload_page_count = parse_uint32_key(header, &ctx, "preload_page_count", 0,
							  &ret);
	if (ret < 0)
		return err_key_parse("header", NULL);

	out->header.fw_image_flags = parse_uint32_hex_key(header, &ctx, "fw_image_flags",
							  SOF_MAN_FW_HDR_FLAGS, &ret);
	if (ret < 0)
		return err_key_parse("header", NULL);

	out->header.feature_mask = parse_uint32_hex_key(header, &ctx, "feature_mask",
							SOF_MAN_FW_HDR_FEATURES, &ret);
	if (ret < 0)
		return err_key_parse("header", NULL);

	out->header.hw_buf_length = parse_uint32_key(header, &ctx, "hw_buf_length", 0, &ret);
	if (ret < 0)
		return err_key_parse("header", NULL);
				  ...
				  bool verbose)
{
	...
	out->write_firmware = man_write_fw_meu_v1_8;
...
static int adsp_parse_config_fd(FILE *fd, struct adsp *out, bool verbose)
{
	...
	if (check_config_version(1, 8, manifest_version)) {
